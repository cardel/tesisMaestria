...
def FSBCAlgorithm(graph,minq,maxq,percentNodesT,repetitions):
	...
	for r in range(0,repetitions):
		...
		for randomCenters in RandomSequences:
			...
			#Select a node in randomlist		
				...
				for center in randomCenters:				
					...
					if nodesMark[int(center)]==0:
						box = numpy.array([], dtype=int)
						box=numpy.append(box,int(center))
						...
						#Review box, the box have to covered nodes into radius r
						if numpy.prod(radiusCovered)==1:
							boxes = numpy.append(boxes,float(numpy.size(box)))
							nodesMark[box] = 1
			...
			for q in range(minq,maxq+1,1):
				...
				Zrq = []
				for ubr in Ub:
					ubQ = numpy.power(ubr,q)/numNodes
					ubQ = numpy.sum(ubQ)
					Zrq.append(ubQ)
				Zq.append(Zrq)
			ZqAverage.append(Zq)
			...
			#Z1e
			for ubr in Ub:
				ubQ = ubr*numpy.log(ubr)/numNodes
				ubQ = numpy.sum(ubQ)
				Z1e.append(ubQ)
		...
		Z1eAverage = numpy.average(Z1eAverage,axis=0)
		##I take average of T
